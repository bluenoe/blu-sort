...
export default function App() {
  ...
  const [summary, setSummary] = useState({ total: 0, duplicates: 0 });
    ...
    <div className="p-6 max-w-6xl mx-auto space-y-6">
      ...
      <section className="card p-4 grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="md:col-span-2 flex items-center gap-3">
          <input
            className="input"
            placeholder="Paste folder path (e.g. E:\\Downloads)"
            value={dir}
            onChange={(e) => setDir(e.target.value)}
          />
          <button className="btn" onClick={scan} disabled={loading || !dir}>
        ...
        <div className="md:col-span-3 text-sm text-zinc-400">
          {summary.total > 0 && (
            <span>
              Preview: {summary.total} items • Duplicates: {summary.duplicates}{" "}
